...
if [ "$MULTI_THREAD_ONLY" = "" ]; then
    /usr/bin/time -p $LAUNCHER $MABOSS cellcycle/cellcycle.bnd -c cellcycle/cellcycle_runcfg.cfg -c cellcycle/cellcycle_runcfg-thread_1.cfg -o tmp/Cell_cycle_thread_1 $FINAL $EXTRA_ARGS
    if [ $? != 0 ]; then exit 1; fi
...
/usr/bin/time -p $LAUNCHER $MABOSS cellcycle/cellcycle.bnd -c cellcycle/cellcycle_runcfg.cfg -c cellcycle/cellcycle_runcfg-thread_6.cfg -o tmp/Cell_cycle_thread_6 $FINAL $EXTRA_ARGS
