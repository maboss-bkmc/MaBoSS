...
/usr/bin/time -p $LAUNCHER mpirun -np $1 $MABOSS_128n ewing/ewing_full.bnd -c ewing/ewing.cfg -c ewing/ewing_runcfg-thread_1.cfg -o tmp/ewing_thread_1
if [ $? != 0 ]; then exit 1; fi
...
/usr/bin/time -p $LAUNCHER mpirun -np $1 $MABOSS_128n ewing/ewing_full.bnd -c ewing/ewing.cfg -c ewing/ewing_runcfg-thread_6.cfg -o tmp/ewing_thread_6
if [ $? != 0 ]; then exit 1; fi
