...
class TestCMaBoSS(TestCase):
    ...
    def test_load_model(self):
        sim = cmaboss_128n.MaBoSSSim(network="models/ewing/ewing_full.bnd", config="models/ewing/ewing.cfg")
        res = sim.run()
        res.get_probtraj()
    ...
    def test_load_model_error(self):
        with self.assertRaises(cmaboss_128n.BNException):
            sim = cmaboss_128n.MaBoSSSim(network="models/ewing/ewing_full-error.bnd", config="models/ewing/ewing.cfg")
    ...
    def test_last_states(self):
        ...
        sim = cmaboss_128n.MaBoSSSim(network="models/metastasis/metastasis.bnd", config="models/metastasis/metastasis.cfg")
        ...
        simfinal = cmaboss_128n.MaBoSSSim(network="models/metastasis/metastasis.bnd", config="models/metastasis/metastasis.cfg")
    ...
    def test_load_model_str(self):
        with open("models/ewing/ewing_full.bnd", "r") as bnd, open("models/ewing/ewing.cfg", "r") as cfg:    
    ...
    def test_load_model_str_error(self):
        with open("models/ewing/ewing_full-error.bnd", "r") as bnd, open("models/ewing/ewing.cfg", "r") as cfg:    
            with self.assertRaises(cmaboss_128n.BNException):
                cmaboss_128n.MaBoSSSim(network_str=bnd.read(),config_str=cfg.read())

    def test_observed_graph(self):
        sim = cmaboss_128n.MaBoSSSim("models/sizek/sizek.bnd", "models/sizek/sizek_wgraph.cfg")
        ...
        values, _ = res.get_observed_durations()
        
        ref_durations = np.array([
            [0.        , 0.85685856, 8.12730648, 0.        , 1.56676983,
                0.        , 0.        , 0.        ],
            [3.44197917, 0.        , 0.        , 1.30921855, 0.        ,
                1.25032368, 0.        , 0.        ],
            [3.2123798 , 0.        , 0.        , 0.77374546, 0.        ,
                0.        , 2.79074152, 0.        ],
            [0.        , 0.88566094, 0.93098387, 0.        , 0.        ,
                0.        , 0.        , 1.12103339],
            [2.3630256 , 0.        , 0.        , 0.        , 0.        ,
                1.30667498, 0.66160254, 0.        ],
            [0.        , 5.12429828, 0.        , 0.        , 3.8799646 ,
                0.        , 0.        , 0.4554656 ],
            [0.        , 0.        , 0.81408612, 0.        , 0.93762464,
                0.        , 0.        , 1.06215587],
            [0.        , 0.        , 0.        , 2.01195064, 0.        ,
                1.0319433 , 1.35525622, 0.        ]
        ])
        
        self.assertTrue(np.allclose(values, ref_durations))
