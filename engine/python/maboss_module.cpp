...
static PyObject* cMaBoSSSim_get_states(cMaBoSSSimObject* self, PyObject* Py_UNUSED) {
  ...
  std::cout << "Timepoints count : " << statedists.size() << std::endl;
  // PyObject *timepoints = PyList_New();
  ...
  for (auto& t_results : statedists) {
    ...
    std::cout << "Adding timepoint " << t_results.first << std::endl;
  ...
  std::cout << "Nb Timepoints : " << PyList_Size(timepoints) << std::endl;
